...
export const ClinicalDecisionPage: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    <div className="flex items-center gap-3 text-sm"><span className={`rounded-full px-3 py-1 font-semibold ${step === 'decision' ? 'bg-blue-700 text-white' : 'bg-slate-100 text-slate-600'}`}>1 Decision</span><span className="h-px w-8 bg-slate-300" /><span className={`rounded-full px-3 py-1 font-semibold ${step === 'review' ? 'bg-blue-700 text-white' : 'bg-slate-100 text-slate-600'}`}>2 Review</span></div>
    ...
    {step === 'decision' ? <Card title="Select the next clinical step">
      ...
      {formError && <p className="mt-4 text-sm font-medium text-rose-700">{formError}</p>}
...
const Notice: React.FC<{ text: string; warning?: boolean }> = ({ text, warning }) => <div className={`flex gap-3 rounded-md border p-3 text-sm ${warning ? 'border-amber-300 bg-amber-50 text-amber-900' : 'border-blue-200 bg-blue-50 text-blue-900'}`}><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />{text}</div>;
const DecisionCard: React.FC<{ selected: boolean; title: string; description: string; icon: React.ReactNode; onClick: () => void }> = ({ selected, title, description, icon, onClick }) => <button type="button" role="radio" aria-checked={selected} onClick={onClick} className={`relative rounded-lg border-2 p-5 text-left transition ${selected ? 'border-blue-700 bg-blue-50' : 'border-slate-200 bg-white hover:border-slate-400'}`}><div className="flex items-center gap-3 font-semibold text-slate-900">{icon}{title}{selected && <span className="ml-auto inline-flex items-center gap-1 text-xs text-blue-700"><Check className="h-4 w-4" />Selected</span>}</div><p className="mt-3 text-sm leading-6 text-slate-600">{description}</p></button>;
