  ...
  if (error || !patient) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        <div className="p-4 bg-red-50 text-red-700 rounded-xl border border-red-200 max-w-md">
          <AlertTriangle className="w-8 h-8 mx-auto mb-2 text-red-500" />
          <h2 className="text-lg font-bold">Document Not Available</h2>
          <p className="text-sm mt-1 text-red-600">{error || 'Patient record could not be found.'}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200">
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-primary-600 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm transition"
          >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
            <div>
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded-full bg-primary-50 text-primary-700 border border-primary-100">
            ...
            {pkg && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-bold text-white bg-primary-600 hover:bg-primary-700 rounded-xl shadow-md transition shrink-0"
              >
        ...
        {pkg && summary && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="bg-green-50/50 border border-green-200/80 rounded-xl p-4 space-y-2">
            ...
            <div className="bg-gradient-to-br from-red-50 to-red-50 rounded-2xl border border-red-200 shadow-sm p-6 sm:p-8 space-y-4">
              ...
              <ul className="grid sm:grid-cols-2 gap-3 text-xs text-red-950">
                {summary.warning_signs && summary.warning_signs.length > 0 ? (
                  summary.warning_signs.map((sign, i) => (
                    <li key={i} className="flex items-start gap-2 bg-white/70 p-3 rounded-lg border border-red-100">
                      <span className="w-1.5 h-1.5 rounded-full bg-red-600 mt-1.5 shrink-0" />
                      <span>{sign}</span>
                    </li>
                  ))
                ) : (
                  <li className="bg-white/70 p-3 rounded-lg border border-red-100">
                    High fever, chest pain, difficulty breathing, or sudden weakness.
                  </li>
                )}
              </ul>

              <div className="pt-2 flex items-center gap-3 text-xs text-red-900 font-semibold bg-red-100/70 p-3 rounded-xl border border-red-200">
